...
function run(commandArgs) {
    ...
    let strykerConfPath = path_1.join(process.cwd(), path || "stryker.conf.js");
    if (!fs_1.existsSync(strykerConfPath)) {
        strykerConfPath = path_1.join(process.cwd(), path || "stryker.conf.json");
    }
    child_process_1.exec(`git rev-parse --verify origin/${branch || "master"}`, (error) => {
        if (error) {
