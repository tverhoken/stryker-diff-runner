...
const stryker_1 = __importDefault(require("stryker"));
const config_1 = require("stryker-api/config");
function run(commandArgs) {
    const strykerConfPath = path_1.join(process.cwd(), "stryker.conf.js");
    child_process_1.exec("git diff origin/master --name-only | grep -E -v '.*\\.test.*' | grep -e 'src/.*\\.ts'", (error, stdout, stderr) => {
        if (error) {
...
function launchStryker(config) {
    const stryker = new stryker_1.default(config);
